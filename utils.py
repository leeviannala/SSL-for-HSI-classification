...
def calprecision(prediction, target, ignored_labels=[], n_classes=None):
    prediction
    target
    row, col = target.shape
    num_class = np.max(target)
    index=np.where(target>0)
    targetavi=target[index[0],index[1]]
    predictionavi = prediction[index[0], index[1]]
    OA=np.sum(targetavi == predictionavi)/len(predictionavi)
    AA_sum=0
    for i in range(1,num_class+1):
        indexi = np.where(targetavi== i)
        targetavi_i= targetavi[indexi[0]]
        predictionavi_i = predictionavi[indexi[0]]
        pre_i=np.sum(targetavi_i == predictionavi_i) / len(predictionavi_i)
        AA_sum = AA_sum + pre_i
    AA=AA_sum/num_class
    return OA*100,AA*100
